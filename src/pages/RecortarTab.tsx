...
export function RecortarTab({ user }: Props) {
  ...
  const apply = async () => {
    if (!videoUrl) return toast.error('Envie um vídeo primeiro.');
    if (!uid) return toast.error('Faça login.');
    setProcessing(true);
    const tid = 'crop-resize';
    toast.loading('Processando o recorte…', { id: tid });
    try {
      ...
      if (!r.ok || !d.url) throw new Error(d.error || 'Falha ao processar.');
      setResultUrl(d.url);
      toast.success('Pronto!', { id: tid });
    } catch (e: any) {
      toast.error(e?.message || 'Erro ao processar.', { id: tid });
    } finally {
      setProcessing(false);
    }
  };
  ...
  const numCls = 'w-24 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 dark:text-gray-100 text-sm';

  return (
    <div className="max-w-3xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        <Crop size={20} className="text-blue-700 dark:text-blue-400" />
        <h2 className="text-lg font-black text-gray-900 dark:text-gray-100">Recortar / Redimensionar</h2>
      </div>
      <p className="text-sm text-gray-500 dark:text-gray-400 -mt-2">
        Arraste a caixa pra recortar exatamente onde quiser, escolha o tamanho de saída (qualquer
        W×H) e, se quiser, corte no tempo.
      </p>

      {!videoUrl ? (
        <label className={box + ' flex flex-col items-center justify-center gap-2 py-12 border-2 border-dashed cursor-pointer text-blue-700 dark:text-blue-300'}>
          {uploading ? <Loader2 size={20} className="animate-spin" /> : <Upload size={20} />}
          <span className="text-sm font-black uppercase tracking-widest">Enviar vídeo</span>
        ...
        <>
          ...
          <div className={box}>
            <div ref={wrapRef} className="relative w-full bg-black rounded-lg overflow-hidden select-none" style={{ touchAction: 'none' }}>
              <video
                src={videoUrl}
                controls
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              Fonte: {srcW}×{srcH}. Recorte: {cropPxW}×{cropPxH}px.
            </p>
          </div>

          {/* Tamanho de saída */}
          <div className={box + ' space-y-2'}>
            <span className="text-[11px] font-black uppercase tracking-widest text-gray-500">Tamanho de saída</span>
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
              <input type="checkbox" checked={lockToCrop} onChange={(e) => setLockToCrop(e.target.checked)} className="accent-blue-600" />
              Usar o tamanho do recorte ({cropPxW}×{cropPxH})
            </label>
            {!lockToCrop && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                Largura
                <input type="number" value={outW} min={2} onChange={(e) => setOutW(Number(e.target.value))} className={numCls} />
                × Altura
                <input type="number" value={outH} min={2} onChange={(e) => setOutH(Number(e.target.value))} className={numCls} />
                px
              </div>
            )}
          </div>
          ...
          <div className={box + ' space-y-2'}>
            <span className="text-[11px] font-black uppercase tracking-widest text-gray-500">
              Cortar no tempo (opcional) · vídeo tem {dur.toFixed(1)}s
            </span>
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
              De
              <input type="number" value={startSec} min={0} step={0.1} onChange={(e) => setStartSec(Math.max(0, Number(e.target.value)))} className={numCls} />
              s até
              <input type="number" value={endSec} min={0} step={0.1} onChange={(e) => setEndSec(Number(e.target.value))} className={numCls} />
              s
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {processing ? <><Loader2 size={16} className="animate-spin" /> Processando…</> : <><Scissors size={16} /> Aplicar</>}
            </button>
            <button
              onClick={() => { setVideoUrl(''); setResultUrl(''); }}
          ...
          {resultUrl && (
            <div className={box + ' space-y-2'}>
              <span className="text-[11px] font-black uppercase tracking-widest text-green-600">Resultado</span>
              <video src={resultUrl} controls className="w-full max-h-[420px] rounded-lg bg-black" />
              <a href={resultUrl} target="_blank" rel="noreferrer" download className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 hover:underline">
