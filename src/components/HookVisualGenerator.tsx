...
const MODELS_IMAGE = [
  { id: 'gemini-imagen', label: 'Gemini Imagen 3', provider: 'Google AI' }
];

const QUALITY_IMAGE = [
  { id: 'fast', label: 'Rápido', model: 'imagen-4.0-fast-generate-001', cost: 0.02, desc: '⚡ ~$0.06 (3 imagens)' },
  { id: 'standard', label: 'Padrão', model: 'imagen-4.0-generate-001', cost: 0.04, desc: '⭐ ~$0.12 (3 imagens)' },
...
const QUALITY_VIDEO = [
  ...
  { id: 'pro', label: 'Pro', model: 'veo-3.1-generate-preview', costPerSec: 0.40, desc: '💎 Ultra Qualidade' }
];
...
export const HookVisualGenerator: React.FC<HookVisualGeneratorProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<StepType>(
    ...
    (hookVisual?.imagemEscolhida) ? 'video-prompt' : 
    (hookVisual?.imagensGeradas?.length > 0) ? 'images' : 
    'choose'
  );
  ...
  const palavrasHook = approvedHook.trim().split(/\s+/).filter(w => w.length > 0).length;
...
Gere the prompt onde o vídeo reforça visualmente a emoção do hook do início ao fim.`;
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      console.log("Operation response completa:", 
    ...
    <div className="w-full max-w-5xl mx-auto p-4 md:p-8 space-y-12">
            ...
            <React.Fragment key={step.id}>
              <div className="flex flex-col items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                  isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'bg-gray-100 text-gray-400'
              ...
              {idx < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-4 mb-6 rounded-full ${
                  idx < STEPS.findIndex(s => s.id === currentStep) ? 'bg-blue-600' : 'bg-gray-100'
                }`} />
              )}
            </React.Fragment>
          );
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="bg-white rounded-[40px] border-2 border-gray-100 shadow-2xl p-8 md:p-12 overflow-hidden relative"
        >
          {/* Step 1: Improve Hook */}
          {currentStep === 'choose' && (
            <div className="space-y-8">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-black text-gray-900 uppercase">A. Escolher Hook</h3>
                </div>
                <p className="text-sm text-gray-500">
                  Escolha um hook da biblioteca, filtre por tipo/tom/nível, ou escreva o seu próprio.
                </p>
              </div>
              ...
              <div className="mt-6 bg-gray-900 rounded-3xl p-6 shadow-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {!sessionHook && (
                    <span className="ml-auto text-xs text-orange-300 font-bold">⚠ Salve um hook primeiro</span>
                  )}
                  {sessionHook && (
                    <span className="ml-auto text-xs text-green-300 font-bold truncate max-w-xs">✅ "{sessionHook.substring(0, 40)}{sessionHook.length > 40 ? '...' : ''}"</span>
                  )}
                </div>
                {!sessionHook && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${sessionHook ? 'bg-blue-100 group-hover:bg-blue-200' : 'bg-gray-700'}`}>
                        <Mic size={18} className={sessionHook ? 'text-blue-600' : 'text-gray-500'} />
                      </div>
                  ...
                  <button
                    ...
                    className={`group rounded-2xl p-5 text-left transition-all border-2 ${sessionHook ? 'bg-white hover:bg-purple-50 border-transparent hover:border-purple-300 hover:shadow-lg cursor-pointer' : 'bg-gray-800 border-gray-700 cursor-not-allowed opacity-50'}`}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${sessionHook ? 'bg-purple-100 group-hover:bg-purple-200' : 'bg-gray-700'}`}>
                        <Film size={18} className={sessionHook ? 'text-purple-600' : 'text-gray-500'} />
                      </div>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-black text-gray-900 uppercase">1. Melhorar Hook</h3>
                </div>
                <div className="p-6 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Hook Aprovado</p>
                  <p className="text-lg font-bold text-gray-700 italic">"{approvedHook}"</p>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 flex-wrap">
                {onProceedToVoice && (
                  <button
                    ...
                    className="px-10 py-5 bg-white border-2 border-gray-200 text-gray-700 rounded-2xl font-black uppercase tracking-widest hover:bg-gray-50 hover:border-gray-300 transition-all shadow-lg flex items-center gap-3"
                  >
                    Ir para Voz Premium
                    <ChevronRight />
                  </button>
            ...
            <div className="space-y-8">
              <div className="space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-black text-gray-900 uppercase">2. Gerar Visual</h3>
                </div>

                <div className="space-y-2">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Prompt da Imagem (Pode editar)</p>
                  <textarea
                    value={improvedImagePrompt}
                    onChange={(e) => setImprovedImagePrompt(e.target.value)}
                    className="w-full p-6 bg-gray-50 rounded-3xl border-2 border-gray-100 focus:border-blue-600 focus:bg-white transition-all text-sm font-medium leading-relaxed min-h-[120px]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Modelo de IA</p>
                      ...
                      {MODELS_IMAGE.map(m => (
                        <button
                          ...
                          className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                            selectedImageModel === m.id ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 hover:border-gray-200'
                          }`}
                        >
                          <div className="text-left">
                            <p className="font-black text-sm text-gray-900 uppercase">{m.label}</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase">{m.provider}</p>
                          </div>
                          {selectedImageModel === m.id && <div className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center text-white"><Check size={12} /></div>}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Qualidade & Custo</p>
                    <div className="grid grid-cols-1 gap-2">
                      {QUALITY_IMAGE.map(q => (
                        <button
                          key={q.id}
                          ...
                          className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                            selectedImageQuality === q.id ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 hover:border-gray-200'
                          }`}
                        >
                          <div className="text-left">
                            <p className="font-black text-sm text-gray-900 uppercase">{q.label}</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase">{q.desc}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-xs font-black text-gray-500">${q.cost}</span>
                            {selectedImageQuality === q.id && <div className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center text-white"><Check size={12} /></div>}
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                <div className="bg-blue-50 p-6 rounded-3xl border-2 border-blue-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="text-blue-600"><Zap size={20} /></div>
                    <div>
                      <p className="text-xs font-black text-blue-900 uppercase">Custo Estimado: <span className="text-blue-600">${getEstimatedCost()}</span></p>
                      <p className="text-[10px] text-blue-800/60 font-bold uppercase">Gerar 3 variações simultâneas</p>
                    </div>
                  </div>
                  <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 border-t border-gray-100">
                    {isGeneratingImages ? (
                      Array(3).fill(0).map((_, i) => (
                        <div key={i} className="aspect-square bg-gray-50 rounded-[32px] animate-pulse flex flex-col items-center justify-center gap-3 border-2 border-dashed border-gray-200">
                          <Loader2 className="animate-spin text-gray-300" size={32} />
                          <span className="text-[10px] font-black text-gray-300 uppercase">Gerando...</span>
                        </div>
                      ))
                    ) : (
                      generatedImages.map((img) => (
                        ...
                        >
                          <img src={img.url || undefined} alt="IA Generated" className="w-full h-full object-cover transition-all group-hover:scale-110" referrerPolicy="no-referrer" />
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-end p-6">
                            <button
                              ...
                              className="w-full py-4 bg-white text-gray-900 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-blue-600 hover:text-white transition-all shadow-xl"
                            >
                              Escolher esta
                            </button>
                          </div>
            ...
            <div className="space-y-8">
              <div className="space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-black text-gray-900 uppercase">3. Produção de Vídeo</h3>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Chosen Image Card */}
                  <div className="space-y-4">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Imagem Escolhida</p>
                    <div className="rounded-[32px] overflow-hidden border-4 border-white shadow-2xl relative aspect-square">
                      {approvedImage && <img src={approvedImage.url || undefined} alt="Imagem aprovada" className="w-full h-full object-cover" referrerPolicy="no-referrer" />}
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-6 bg-blue-50 rounded-3xl border-2 border-blue-100">
                      <p className="text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">✅ Duração do vídeo: {duracaoVideo}s</p>
                      <p className="text-[10px] text-blue-800 font-bold uppercase">{palavrasHook} palavras · ~{duracaoVideo}s</p>
                    </div>

                    <div className="space-y-4">
                      {/* Formato / Proporção Select */}
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Formato</p>
                          ...
                          {ASPECT_RATIOS.map(a => (
                            <button
                              ...
                              className={`py-2 rounded-xl border-2 text-[10px] font-black transition-all ${
                                selectedAspectRatio === a.id ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-100 text-gray-400 hover:border-gray-200'
                              }`}
                            >
                              {a.id}
                            </button>
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full p-6 bg-gray-50 rounded-[32px] border-2 border-gray-100 focus:border-blue-600 focus:bg-white transition-all text-sm font-medium leading-relaxed min-h-[160px]"
                    ...
                    {/* Quality Selector */}
                    <div className="space-y-3">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Qualidade de Vídeo</p>
                      <div className="grid grid-cols-1 gap-2">
                        {QUALITY_VIDEO.map(q => (
                          <button
                            key={q.id}
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                              selectedVideoQuality === q.id ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 hover:border-gray-200'
                            }`}
                          >
                            <div className="text-left">
                              <p className="font-black text-xs text-gray-900 uppercase">{q.desc}</p>
                              <p className="text-[10px] text-gray-400 font-bold uppercase">{q.id}</p>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="text-[10px] font-black text-gray-500">{duracaoVideo}s = ~${(duracaoVideo * q.costPerSec).toFixed(2)}</span>
                              {selectedVideoQuality === q.id && <div className="w-4 h-4 bg-blue-600 rounded-full flex items-center justify-center text-white"><Check size={10} /></div>}
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    {isGeneratingVideo && (
                      <div className="space-y-3 p-6 bg-gray-900 rounded-[32px] text-center">
                        <div className="flex items-center justify-center gap-3 mb-2">
                          ...
                          <span className="text-[10px] font-black text-white uppercase tracking-widest">Produzindo vídeo... {Math.round(videoGenerationProgress)}%</span>
                        </div>
                        ...
                        <p className="text-[10px] text-gray-500 font-medium italic">Aguarde, sua copy visual está sendo preparada.</p>
                      </div>
                    )}
                    ...
                    {!isGeneratingVideo && (
                      <div className="grid grid-cols-2 gap-4">
                        <button
                          ...
                          className="py-6 bg-white border-2 border-blue-600 text-blue-600 rounded-[32px] font-black text-sm flex items-center justify-center gap-2 shadow-xl transition-all hover:bg-blue-50 active:scale-95 disabled:opacity-50"
                        >
          ...
          {currentStep === 'done' && (
            <div className="space-y-12 py-6 text-center">
              <div className="space-y-4">
                <div className="w-20 h-20 bg-green-50 text-green-600 rounded-[32px] flex items-center justify-center mx-auto shadow-xl shadow-green-100">
                  <Check size={40} />
                </div>
                <h3 className="text-3xl font-black text-gray-900 uppercase tracking-tighter">Produção Finalizada!</h3>
      ...
      {imageHistory.length > 0 && currentStep !== 'done' && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-sm font-black text-gray-900 uppercase tracking-widest">Histórico de Imagens</h4>
          </div>
            ...
            {imageHistory.map((img) => (
              <div 
                ...
                className={`flex-shrink-0 w-24 h-24 rounded-2xl overflow-hidden cursor-pointer border-4 transition-all ${
                  approvedImage?.id === img.id ? 'border-blue-600 ring-4 ring-blue-50 scale-105' : 'border-white shadow-md'
                }`}
              >
